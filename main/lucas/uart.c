...
esp_err_t lucas_uart_init() {
    ...
    LUCAS_ESP_TRY(uart_set_pin(LUCAS_UART_PORT, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    LUCAS_ESP_TRY(uart_driver_install(LUCAS_UART_PORT, LUCAS_UART_BUFFER_SIZE, LUCAS_UART_BUFFER_SIZE, 10, &ctx.event_queue, 0));
